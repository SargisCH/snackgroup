...
const Contact = () => {
  ...
  return (
    <>
      ...
      `}</style>
      <section id="contact" style={{ paddingTop: '80px', minHeight: '50vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', color: '#8B4513' }}>
        <h2 style={{ color: '#8B4513' }}>{t('contact')}</h2>
        <p style={{ color: '#8B4513' }}>{t('contact_us_intro')}</p>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '400px', marginTop: '2rem' }}>
          ...
          <button type="submit" className="submit-btn" style={{ padding: '0.75rem', backgroundColor: '#8B4513', color: 'white', border: 'none', cursor: 'pointer', borderRadius: '4px', transition: 'background-color 0.3s' }}>{t('send')}</button>
        </form>
      </section>
    </>
  );
